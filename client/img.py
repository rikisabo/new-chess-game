...
class Img:
    ...
    def read(self, path: str | pathlib.Path,
        ...
        # Handle Hebrew/Unicode characters in path for OpenCV
        try:
            # Try to read with numpy and cv2.imdecode for Unicode paths
            import numpy as np
            with open(path, 'rb') as f:
                file_bytes = np.frombuffer(f.read(), np.uint8)
            self.img = cv2.imdecode(file_bytes, cv2.IMREAD_UNCHANGED)
        except Exception as e:
            print(f"Unicode path method failed: {e}")
            # Fallback to regular cv2.imread
        ...
        if self.img is None:
            ...
            try:
                import numpy as np
                # Try encoding the path differently
    ...
    def show(self):
        ...
        # Check for ESC key to exit gracefully
