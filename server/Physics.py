...
class BasePhysics(ABC):  # Interface/base class
    ...
    def set_position(self, cell_position: list):
        """Update piece position to specific cell"""
        if len(cell_position) >= 2:
            new_pos_m = self.board.cell_to_m((cell_position[0], cell_position[1]))
            self._curr_pos_m = new_pos_m
